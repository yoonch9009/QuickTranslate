...
트레이 아이콘을 우클릭하고 `원클릭 업데이트`를 선택하세요. 최신 정식 GitHub 릴리즈를
확인하고 EXE를 내려받아 파일 크기와 SHA-256을 검증한 뒤 교체·재실행합니다.
다운로드 중에는 취소할 수 있으며, 진행 중인 번역은 먼저 완료해야 합니다.

설정은 `%APPDATA%\QuickTranslate`에 유지됩니다. 기존 EXE는 같은 위치의
`QuickTranslate.exe.previous`에 보관되며, 교체 후 실행 명령이 실패하면 복원합니다.
새 버전 실행 이후의 모든 런타임 오류를 자동 복구하는 기능은 아닙니다.
실행 파일이 있는 폴더에 쓰기 권한이 필요합니다.
업데이트 실패 시 표시되는 오류를 확인하고 릴리즈 페이지에서 수동 다운로드할 수 있습니다.

1.7.1 이하에는 업데이트 메뉴가 없으므로 최초 한 번은 새 EXE를 직접 내려받아 교체하세요.
소스로 실행할 때는 Git으로 업데이트합니다.
